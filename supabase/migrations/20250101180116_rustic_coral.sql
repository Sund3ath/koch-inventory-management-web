/*
  ...
  1. New Functions
    - get_dashboard_metrics: Main function to fetch all dashboard metrics
    - get_device_metrics: Calculate device-related metrics
    - get_license_metrics: Calculate license-related metrics
    - get_employee_metrics: Calculate employee-related metrics
    - get_department_metrics: Calculate department-related metrics
...
BEGIN
  SELECT json_build_object(
    ...
    'utilization', (
      SELECT COALESCE(
        ROUND(
          (COUNT(DISTINCT la.id)::FLOAT / NULLIF(SUM(l.total_quantity), 0) * 100),
          2
        ),
        0
      )
...
BEGIN
  SELECT json_build_object(
    ...
    'by_department', (
      SELECT json_object_agg(d.name, count)
      FROM (
    ...
    'device_allocation', (
      SELECT ROUND(
        (COUNT(DISTINCT da.device_id)::FLOAT / NULLIF(COUNT(DISTINCT e.id), 0)),
        2
      )
      FROM employees e
...
BEGIN
  SELECT json_build_object(
    ...
    'device_distribution', (
      SELECT json_object_agg(d.name, device_count)
      FROM (
